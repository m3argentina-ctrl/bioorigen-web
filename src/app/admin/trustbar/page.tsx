...
export default function TrustBarAdminPage() {
  ...
  async function toggleActive(item: Item) {
    const res = await fetch(`/api/admin/trustbar/${item.id}`, {
      method: "PUT",
  ...
  async function handleDelete(id: string) {
    ...
    await fetch(`/api/admin/trustbar/${id}`, { method: "DELETE" });
    load();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Barra de confianza</h1>
          <p className="mt-1 text-sm text-slate-500">
            Íconos con título y texto que aparecen debajo del menú de navegación (ej: "Envíos a todo el país").
          </p>
        </div>
        <Link
          href="/admin/trustbar/nuevo"
            ...
            <tbody className="divide-y divide-slate-100">
              {sorted.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="max-w-xs truncate px-4 py-3 text-slate-500">{item.text}</td>
                  <td className="px-4 py-3">
                    <button
                      type="button"
