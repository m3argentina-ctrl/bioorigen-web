...
export default function EditTrustBarItemPage({ params }: { params: { id: string } }) {
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ icon: "Truck", title: "", text: "", order: 0, active: true });

  useEffect(() => {
    fetch("/api/admin/trustbar")
      .then((r) => r.json())
      .then((items: { id: string; icon: string; title: string; text: string; order: number; active: boolean }[]) => {
        const item = items.find((i) => i.id === params.id);
        if (item) setForm({ icon: item.icon, title: item.title, text: item.text, order: item.order, active: item.active });
      });
  }, [params.id]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    const res = await fetch(`/api/admin/trustbar/${params.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...form, order: Number(form.order) }),
    });
    if (res.ok) router.push("/admin/trustbar");
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/trustbar" className="rounded p-1 text-slate-400 hover:text-slate-700">
          <ArrowLeft size={20} />
        </Link>
        <h1 className="text-2xl font-bold text-slate-900">Editar ítem barra de confianza</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-xl border border-slate-200 bg-white p-6">
        ...
        <div className="flex justify-end gap-3">
          <Link href="/admin/trustbar" className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
            Cancelar
          </Link>
