...
export default function EditTopBarItemPage({ params }: { params: { id: string } }) {
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ icon: "CreditCard", text: "", order: 0, active: true });

  useEffect(() => {
    fetch("/api/admin/topbar")
      .then((r) => r.json())
      .then((items: { id: string; icon: string; text: string; order: number; active: boolean }[]) => {
        const item = items.find((i) => i.id === params.id);
        if (item) setForm({ icon: item.icon, text: item.text, order: item.order, active: item.active });
      });
  }, [params.id]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    const res = await fetch(`/api/admin/topbar/${params.id}`, {
      method: "PUT",
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-slate-900">Editar ítem barra superior</h1>
      </div>
